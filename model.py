"""
@Author: Copy from Torch[@Github] for pretrained model
@Data  : 20210911
"""
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(self.input_channel, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
def _resnet(
    ...
    **kwargs: Any     # 可以在这里改 `num_classes`
    ...
    if pretrained:
        ...
        #                                       progress=progress)
        if arch == "resnet50":
            pth_file = "model_pretrained/resnet50-0676ba61.pth"
        else:
            raise NotImplementedError("( •̀ ω •́ )y  我还没有实现嘿嘿")
...
class TianChiModel(nn.Module):
    ...
    def __init__(self, model_name, input_channel=3,  class_out=4, regr_out=3, pretrained=True):
        ...
        if not model_name in self.model_zoo:
            # 要是换模型我就把下面羞耻的话注释掉......
